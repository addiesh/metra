...
fn run_rustfmt(code: String) -> io::Result<String> {
	let rustfmt = if let Ok(rustfmt) = env::var("RUSTFMT") {
		...
		which::which("rustfmt")
			.expect("Failed to find rustfmt using \"which\" !")
	};
...
fn main() {
	...
	let assets_dir = fs::read_dir(&assets_path)
		.expect("Failed to find asset directory!");

	{
		let canonical = assets_path.canonicalize().unwrap().to_string_lossy().to_string();
	...
	while !path_stack.is_empty() {
		...
		let category_name: Option<String> = dir_path.as_ref().map(|dir|
			dir.file_name().unwrap().to_string_lossy()
				.to_lowercase()
				.chars()
				.filter_map(|ch| if ch == '_' || ch.is_alphanumeric() {
					Some(ch)
				} else if ch == '-' || ch == '.' {
					Some('_')
				} else {
		...
		if let Some(dir_path) = &dir_path {
			let canonical = dir_path.canonicalize().unwrap().to_string_lossy().to_string();
			// println!("cargo::rerun-if-changed={canonical}");
		}

		// ignore directories with a .ignore_assets file
		if entries.iter().find(|de| {
		...
		for entry in entries {
			...
			let name: String = entry.path().file_stem().unwrap().to_string_lossy()
				.to_uppercase()
				.chars()
				.filter_map(|ch| if ch.is_alphanumeric() {
					Some(ch)
				} else if ch == '_' || ch == '-' || ch == '.' {
					Some('_')
				} else {
				...
				.collect();
			let canonical: String = entry.path().canonicalize().unwrap().to_string_lossy().into();
			// println!("cargo::rerun-if-changed={canonical}");
	...
	out_string = run_rustfmt(out_string)
		.expect("Failed to format generated code!");

	let gen_src_path: PathBuf = [&env::var("OUT_DIR").unwrap(), "assets.rs"].iter().collect();
	fs::write(gen_src_path, out_string)
		.unwrap()
}