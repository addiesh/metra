...
// 
// 	pub fn new_mesh_temp(&mut self) -> Mesh<'a> {
// 		Mesh {
...
// pub struct Effect { }
// 
// pub fn run<'engine: 'state, 'state: 'temp, 'temp, T: 'state>(
// 	init: fn(&'temp mut Metro) -> T,
// 	update: fn(state: &mut T, metro: &mut Metro) -> bool
// ) {
// 	fn tick<'engine: 'state, 'state: 'temp, 'temp, T: 'state>(
// 		game_state: &'temp mut T,
// 		metro: &'temp mut Metro,
// 		update: fn(state: &'temp mut T, metro: &'temp mut Metro) -> bool
// 	) {
...
// 	let context = 123;
// 
// 	let mut metro = Metro::new(&context);
// 
